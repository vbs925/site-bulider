...
import { Loader2Icon, LayoutTemplate, Aperture, Instagram, LayoutGrid, Sparkles, Hexagon } from 'lucide-react'
...
const Home = () => {
    ...
    return (
        <section className="flex flex-col items-center text-white text-sm pb-20 px-4 font-poppins">
            ...
            <form onSubmit={onSubmitHandler} className="bg-white/10 max-w-2xl w-full rounded-xl p-4 mt-10 border border-indigo-600/70 focus-within:ring-2 ring-indigo-500 transition-all">
                ...
                <button className="ml-auto flex items-center gap-2 bg-gradient-to-r from-[#CB52D4] to-indigo-600 rounded-md px-4 py-2">
                    {!loading ? ' Create with AI' : (
                        <>
                            Creating your website...<Loader2Icon className='animate-spin size-4 text-white' />
                        </>
                    )}
            ...
            <div className="flex flex-wrap items-center justify-center gap-10 md:gap-14 mx-auto mt-16 text-gray-400 font-medium">
                {/* Framer */}
                <div className="flex items-center gap-2 hover:text-white transition-colors duration-300">
                    <LayoutTemplate className="size-8" />
                    <span className="text-xl">Framer</span>
                </div>

                {/* Huawei - using Aperture as flower-like logo */}
                <div className="flex items-center gap-2 hover:text-white transition-colors duration-300">
                    <Aperture className="size-8" />
                    <span className="text-xl">Huawei</span>
                </div>
                ...
                {/* Walmart - using Sparkles as starburst */}
                <div className="flex items-center gap-2 hover:text-white transition-colors duration-300">
                    <Sparkles className="size-8" />
                    <span className="text-xl">Walmart</span>
                </div>
            </div>
        </section>
