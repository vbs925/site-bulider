    ...
    return project ? (
        <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
            ...
            <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar border-b border-gray-700'>
                ...
                {/*middle*/}
                <div className='flex items-center justify-center gap-2 bg-gray-800 border border-gray-700 rounded-md p-1'>
                    <Smartphone onClick={() => setDevice('phone')} className={`size-6 p-1 rounded cursor-pointer hover:bg-gray-700 
                        ${device === 'phone' ? 'bg-gray-700' : ''}`} />
                    <TabletIcon onClick={() => setDevice('tablet')} className={`size-6 p-1 rounded cursor-pointer hover:bg-gray-700 
                        ${device === 'tablet' ? 'bg-gray-700' : ''}`} />
                    <LaptopIcon onClick={() => setDevice('desktop')} className={`size-6 p-1 rounded cursor-pointer hover:bg-gray-700
                ...
                <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
                    ...
                    flex flex-col items-center gap-1 rounded sm:rounded-sm transition-colors border border-gray-700'>
                        {isSaving ? <Loader2Icon className="animate-spin" size={16} /> : <SaveIcon size={16} />} Save
                    </button>
                    <Link target='_blank' to={`/preview/${project.id}`} className='flex flex-col items-center gap-1 px-4 py-1 sm:rounded-sm border border-gray-700
